...
const {
  faker: { USER_ID, ORGANIZATION_ID, CATEGORY_ID, TYPE_IDS, MIN_PHOTO, MAX_PHOTO, PLACES_SEEDS },
  places: {
    default: { DAYS },
  },
} = require('../../config');
...
function getPhotos() {
  return new Array(faker.random.number({ min: MIN_PHOTO, max: MAX_PHOTO })).map(() => {
...
const place = {
  ...
  place: {
    ...
    category_id: faker.random.arrayElement(CATEGORY_ID),
...
module.exports = { organization, place };
